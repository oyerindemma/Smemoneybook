...
| Cooperatives | `/more/cooperatives` | C. PARTIALLY IMPLEMENTED | `NEXT_PUBLIC_PHASE3_COOPERATIVES_ENABLED` | Pro | none | `money:write`; location access on scoped writes | `CooperativeGroup`, `CooperativeMember`, contribution, loan, repayment, meeting, resolution, expense, distribution, and ledger models | none | `src/app/api/cooperatives/route.test.ts`, `src/lib/phase3/cooperative-ledger.test.ts` | API covers groups, members, contributions, loans, and repayments; UI only exposes create group/add member and summary. | More label changed to `Coming soon` while disabled. | Pending post-deploy smoke |
