...
import { rejectCooperativeLoan } from "@/lib/cooperatives/loans";
import { loanRejectSchema } from "@/lib/cooperatives/validation";
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const body = await parseJsonBody(request, loanRejectSchema);
    const limited = await enforceRateLimit(request, "cooperatives.loans.reject", 20, 60 * 60 * 1000);
    ...
    const loan = await rejectCooperativeLoan({
    ...
    await logCooperativesAudit({
      access,
      action: "cooperatives.loan_rejected",
      metadata: { groupId: id, loanId },
    });

    return Response.json({ loan, capabilities: cooperativesCapabilityPayload(access) });
  } catch (error) {
    return cooperativesErrorResponse(error, "Could not reject cooperative loan.");
  }
}
