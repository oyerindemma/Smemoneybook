...
import { loanRepaymentSchema } from "@/lib/cooperatives/validation";
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const body = await parseJsonBody(request, loanRepaymentSchema);
    const limited = await enforceRateLimit(request, "cooperatives.loans.repay", 40, 60 * 60 * 1000);
    ...
    const access = await requireCooperativesAccess({
      ...
      permission: "cooperatives:record_repayment",
    });
    const repayment = await recordCooperativeLoanRepayment({
      ...body,
    ...
    await logCooperativesAudit({
      access,
      action: "cooperatives.loan_repayment_recorded",
      metadata: { groupId: id, loanId, repaymentId: repayment.id },
    });
    ...
    return cooperativesErrorResponse(error, "Could not record cooperative loan repayment.");
  }
}
