...
import { rejectStockTransferForUser } from "@/lib/transfers/service";
...
export async function POST(
  ...
  try {
    ...
    const limited = await enforceRateLimit(request, "stock_transfers.reject", 40, 15 * 60 * 1000);

    if (limited) {
      return limited;
    }

    const transfer = await rejectStockTransferForUser({
      userId: user.id,
    ...
    console.error(error);
    return jsonErrorFromUnknown(error, "Could not reject transfer.");
  }
}
