...
import { sendStockTransferForUser } from "@/lib/transfers/service";
...
export async function POST(
  ...
  try {
    ...
    const limited = await enforceRateLimit(request, "stock_transfers.send", 40, 15 * 60 * 1000);

    if (limited) {
      return limited;
    }

    const transfer = await sendStockTransferForUser({
      userId: user.id,
    ...
    console.error(error);
    return jsonErrorFromUnknown(error, "Could not send transfer.");
  }
}
