...
import { cancelStockTransferForUser } from "@/lib/transfers/service";
...
export async function POST(
  ...
  try {
    ...
    const access = await requireBusinessAccess(user.id, "transfers:cancel", body.businessId);
    ...
    const limited = await enforceRateLimit(request, "stock_transfers.cancel", 40, 15 * 60 * 1000);

    if (limited) {
      return limited;
    }

    const transfer = await cancelStockTransferForUser({
      userId: user.id,
    ...
    console.error(error);
    return jsonErrorFromUnknown(error, "Could not cancel transfer.");
  }
}
