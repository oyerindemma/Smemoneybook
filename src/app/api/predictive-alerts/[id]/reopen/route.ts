...
export async function POST(
  ...
  try {
    ...
    const limited = await enforceRateLimit(request, "predictive_alerts.reopen", 30, 60 * 60 * 1000);
    ...
    await logPredictiveAlertsAudit({
      access,
      action: "predictive_alerts.reopened",
      metadata: { alertId: id, ruleKey: alert.ruleKey },
    });

    return Response.json({ alert, message: "Predictive Alert reopened." });
  } catch (error) {
    return predictiveAlertsErrorResponse(error, "Could not reopen Predictive Alert.");
  }
}
